...
const Requested = () => {
  ...
  const userDatas = [
    { name: "John Doe", age: 30, email: "johndoe@example.com", status: "Accepted" },
    { name: "Jane Doe", age: 25, email: "janedoe@example.com", status: "Pending" },
    { name: "Jim Beam", age: 50, email: "jimbeam@example.com", status: "Rejected" },
    { name: "Jack Daniels", age: 65, email: "jackd@example.com", status: "Accepted" },
    { name: "Jill Stein", age: 42, email: "jillstein@example.com", status: "Pending" }
  ];
  ...
  const getStatusIcon = (status) => (
    <div className="flex items-center space-x-2"> {/* Ensures horizontal layout and space between items */}
      <div className={`w-3 h-3 rounded-full ${status === 'Accepted' ? 'bg-green-500' : status === 'Rejected' ? 'bg-red-500' : status === 'Pending' ? 'bg-yellow-500' : 'bg-gray-300'}`}></div>
      <span>{status}</span> {/* Displays the status text next to the circle */}
    </div>
  );
    ...
    <div>
      ...
      <div className="w-full p-4 mt-3 rounded-xl shadow-xl bg-white overflow-auto max-h-[calc(100vh-200px)]">
        ...
        <div className="w-full grid grid-cols-5 gap-4 p-4 bg-blue-100 rounded-full items-center">
          ...
          <h4 className="text-gray-800 font-medium ">Status</h4>
